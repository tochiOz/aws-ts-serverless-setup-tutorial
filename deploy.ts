...
console.log(
  `Deploying App=${options.application ? options.application : 'ALL'}, Stage=${options.stage}, Region=${options.region}, Profile=${options.profile}`
);

// Get AWS Account ID
const getAccountIdCmd = `aws sts get-caller-identity --output text --query 'Account' --profile ${options.profile}`;
const awsAccountId = shell.exec(getAccountIdCmd, { silent: true }).stdout.replace(/\n$/, '');
...
// Deploy Apps
const APPS = ['db', 'messaging', 'Others'];
...
appsToDeploy.map(application => {
  ...
  const deployCmdBucket = `aws s3api create-bucket --bucket "${options.region}.${awsAccountId}.deploys.${application}" --region=${options.region} --profile=${options.profile}`;
